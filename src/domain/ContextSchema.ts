...
export class ContextSchema {
  ...
  public filterContext(context: Context): Context {
    return new Context(
      context.Keys.filter(key =>
        this.attributes.some(attr => attr.Id === key)
      ).reduce((accu, curr) => ({ ...accu, [curr]: context.get(curr) }), {})
    );
  }
  ...
  private validateContext(context: Context): void {
    ...
    const unknownAttrs = new Set(
      context.Keys.filter(contKey => !attrIdSet.has(contKey))
    );
  ...
  public getMostRelevant(toggles: Toggle[]): Toggle {
    ...
    weightedToggles.forEach(weightedToggle => {
      const other = weightedToggles.find(
        o => o.Weight === weightedToggle.Weight
      );
